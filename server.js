...
if (totalEquity > 0) {
    await pool.query(
        `INSERT INTO portfolio_snapshots (user_id, total_value, mode, recorded_at) 
         VALUES ($1, $2, $3, NOW())`,
        [record.user_id, totalEquity, mode]
    );
    console.log(`? [${mode.toUpperCase()}] Snapshot saved: $${totalEquity.toFixed(2)}`);
}
...
cron.schedule('0 * * * *', async () => {
    try {
        await pool.query("DELETE FROM portfolio_snapshots WHERE recorded_at < NOW() - INTERVAL '24 hours'");
